...
const progress = (() => {
    ...
    const progress = () => {
        ...
        info.innerText = `Now Loading... (${loaded}/${total}) [${parseInt(bar.style.width).toFixed(0)}%]`;
...
fetch(api_url)
    ...
    .then(data => {
        ...
        for (let a of data.data) {
            const apiDataContainer = document.getElementById('span-one');
            apiDataContainer.innerHTML = `${a.name}`; // Replace with actual property from API response
        }
        for (let a of data.data) {
        ...
        for (let a of data.data) {
            ...
            apiDataContainer.innerHTML = `${a.message}`;
        }
        for (let a of data.data) {
            const apiDataContainer = document.getElementById('span-four');
            apiDataContainer.innerHTML = `${a.created_at}`;
        }
    })
    .catch(error => {
